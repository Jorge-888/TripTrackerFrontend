...
export class EditComponent implements OnInit {
    @Input() viajeToEdit: any;
    @Output() updated = new EventEmitter<Viaje>();
    ...
    tarifaChanged: boolean = false;
    originalTarifaKM: number = 0;
    ...
    private loadViajeData(): void {
        this.viaje = {
            viaj_Id: this.viajeToEdit.viaj_Id,
            viaj_Fecha: new Date(this.viajeToEdit.viaj_Fecha),
            trpo_Id: this.viajeToEdit.trpo_Id,
            sucu_Id: this.viajeToEdit.sucu_Id,
            viaj_Estado: this.viajeToEdit.viaj_Estado,
            viaj_TarifaKM: this.viajeToEdit.viaj_TarifaKM,
            usua_Creacion: this.viajeToEdit.usua_Creacion,
            viaj_FechaCreacion: this.viajeToEdit.viaj_FechaCreacion
        };

        this.originalTarifaKM = this.viajeToEdit.viaj_TarifaKM;

        this.selectedTransportista = this.transportistas.find(t => t.trpo_Id === this.viaje.trpo_Id) || null;

        if (this.selectedTransportista) {
            this.tarifaChanged = this.selectedTransportista.trpo_TarifaKM !== this.originalTarifaKM;
        }
    ...
    private preSelectColaboradores(): void {
        if (!this.viajeToEdit.colaboradoresJson) return;

        try {
            const selectedColabs = JSON.parse(this.viajeToEdit.colaboradoresJson);

            this.colaboradores.forEach(colab => {
                const wasSelected = selectedColabs.some((sc: any) => sc.Colb_Id === colab.colb_Id);
                if (wasSelected) {
                    colab.selected = true;
                }
            });
    ...
    private checkColaboradorDisponibilidad(): void {
        ...
        this.colaboradores.forEach(colaborador => {
            const viajeEnFecha = this.viajesExistentes.find(viaje => {
                if (viaje.viaj_Id === this.viaje.viaj_Id) return false;
    ...
    private calculateTarifaTotal(): void {
        if (this.totalDistancia > 0) {
            this.tarifaTotal = this.totalDistancia * this.originalTarifaKM;
        } else {
            this.tarifaTotal = 0;
        }
    }
    ...
    submit(form: NgForm): void {
        ...
        const viajeData = {
            viaj_Id: this.viaje.viaj_Id,
            viaj_Fecha: this.formatLocalDateTime(this.viaje.viaj_Fecha),
            viaj_TarifaTotal: this.tarifaTotal,
            viaj_TarifaKM: this.originalTarifaKM,
            trpo_Id: this.viaje.trpo_Id,
            sucu_Id: this.viaje.sucu_Id,
            viaj_Estado: true,
            usua_Creacion: this.viaje.usua_Creacion,
            viaj_FechaCreacion: this.viaje.viaj_FechaCreacion,
        ...
        this.http.put<Viaje>(`${environment.apiUrl}/Viajes/ActualizarViajesCompleto`, viajeData, {
            headers: { 'x-api-key': environment.API_KEY }
        }).subscribe({
            next: updated => {
                this.saving = false;
                this.toastr.success('Viaje actualizado exitosamente', 'Éxito');
                this.updated.emit(updated);
            },
            error: err => {
                this.saving = false;
                console.error('Error al actualizar viaje', err);
                this.toastr.error('No se pudo actualizar el viaje. Intenta de nuevo.', 'Error');
            }
        });
    }
